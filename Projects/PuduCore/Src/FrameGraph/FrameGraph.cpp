...
#pragma endregion
    ...
    static bool IsNodeDependant(FrameGraph* graph, FrameGraphNodeHandle childNode, FrameGraphNodeHandle targetNode)
    {
        ...
        for (const auto inputEdge : n->inputEdges)
        {
            const auto edge = graph->GetNodeEdge(inputEdge);
    ...
    static void ComputeEdges(FrameGraph* frameGraph)
    {
        for (size_t i = 0; i < frameGraph->nodes.size(); i++)
        {
            ...
            for (int j = i - 1; j >= 0; j--)
            {
                ...
                //Straightforward dependency case, parent node writes to input resource
                for (auto parentOutput : parentNode->outputs)
                {
                    for (auto input : node->inputs)
                    {
                        if (input.resource->Handle().IsEqual(parentOutput.resource->Handle()))
                        {
                            ...
                            {
                                auto edgeHandle = frameGraph->builder->CreateNodeEdge(
                                    parentNodeHandle, nodeHandle, input.resource->Handle());
                                ...
                                std::printf("Edge: {%s} {%i}->{%s} {%i} \n", parentNode->name.c_str(), edge->from,
                                            node->name.c_str(), edge->to);
                            }
                        }
                    }
                }

                //Usage dependency, for textures if the resource usage is different then a barrier changing the layout must be inserted, therefore creating a dependency
                for (auto parentInput : parentNode->inputs)
                {
                    for (auto input : node->inputs)
                    {
                        if (input.type == GPUResourceType::Texture && input.resource->Handle().IsEqual(
                            parentInput.resource->Handle()))
                        {
                            if (input.usage != parentInput.usage)
                            {
                                if (!IsNodeDependant(frameGraph, node->nodeHandle, parentNode->nodeHandle))
                                {
                                    auto edgeHandle = frameGraph->builder->CreateNodeEdge(
                                    ...
                                    std::printf("Edge (usage): {%s} {%i}->{%s} {%i} \n", parentNode->name.c_str(),
                                                edge->from, node->name.c_str(), edge->to);
                                }
                            }
                        }
                    }
    ...
    GPUResourceBase* FrameGraphBuilder::GetResource(GPUResourceHandleBase textureHandle)
    {
        ...
        return graphics->Resources()->GetTexture<RenderTexture>(static_cast<GPUResourceHandle<Texture>>(textureHandle)).
                         get();
    }
    ...
    void FrameGraph::AllocateResource(GPUResourceHandleBase handle) const
    {
        const auto r = builder->GetResource(handle);
    ...
    void FrameGraph::AllocateRequiredResources() const
    {
        LOG("FrameGraph: Allocating Resources");
        for (const auto nodeHandle : nodes)
        {
            const auto node = builder->GetNode(nodeHandle);

            for (const auto outputResource : node->outputs)
            {
                AllocateResource(outputResource.resource->Handle());
            }

            for (const auto inputResource : node->inputs)
            {
    ...
    void FrameGraph::Compile()
    {
        ...
        for (FrameGraphNodeHandle nodeHandle : nodes)
        {
            ...
            while (nodesToBeVisitedStack.size() > 0)
            {
                ...
                //Add child handles
                for (const auto edgeHandle : node->outputEdges)
                {
                    const auto edge = GetNodeEdge(edgeHandle);
    ...
    void FrameGraph::RenderFrame(RenderFrameData& renderData)
    {
        ...
        for (auto nodeHandle : nodes)
        {
            ...
            for (const auto& outputResource : node->outputs)
            {
                ...
                {
                    const auto texture = outputResource.resource;
            ...
            for (const auto& inputResource : node->inputs)
            {
                ...
                {
                    const auto texture = inputResource.resource;
    ...
    ResourceUsage FrameGraph::GetTextureUsage(const GPUResourceHandleBase handle) const
    {
        if (m_usageByTexture.contains(handle))
        {
            return m_usageByTexture.at(handle);
        }
        return UNDEFINED;
    }

    void FrameGraph::SetTextureUsage(const GPUResourceHandleBase handle, const ResourceUsage usage)
    {
        m_usageByTexture[handle] = usage;
    }

    std::string FrameGraph::ToString() const
    {
        std::stringstream ss;
    ...
    FrameGraphNode* FrameGraph::GetNode(const char* name) const
    {
        return builder->GetNode(name);
    }

    FrameGraphNode* FrameGraph::GetNode(const FrameGraphNodeHandle nodeHandle) const
    {
        return builder->GetNode(nodeHandle);
    }

    NodeEdge* FrameGraph::GetNodeEdge(const NodeEdgeHandle edgeHandle) const
    {
    ...
    {
        const auto renderPass = builder->graphics->Resources()->GetRenderPass(creationData.renderPass);
        ...
        {
            const auto attachment = renderPass->attachments.depthAttachments[0];
            creationData.inputs.push_back(attachment);
        ...
        const auto handle = builder->CreateNode(creationData);
    ...
    VkAttachmentLoadOp GetVkAttachmentLoadOp(const LoadOperation op)
    {
        switch (op)
        {
        case Pudu::DontCare:
